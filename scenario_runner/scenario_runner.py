...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        logging.info(' ScenarioRunner::__init__() %d', int(self._args.trafficManagerPort))
        ...
        #store our pid in a tmp file
        sc_pid = os.getpid()
        sc_pid_file = tmpd() + '/sc.pid'
        scd = os.open(sc_pid_file.replace(os.sep, '/'),os.O_CREAT|os.O_TRUNC|os.O_RDWR)
        os.lseek(scd, 0, os.SEEK_SET)
        # initialise the file with zeros
        os.write(scd, str.encode(str(sc_pid)))
        logging.debug("ScenarioRunner pid %d saved in %s", sc_pid, sc_pid_file)
    ...
    def _signal_handler(self, signum, frame):
        ...
        if self.manager:
            ...
            logging.debug("Terminating SC due to received signal %d", signum)
            self._cleanup()
    ...
    def _get_scenario_class_or_fail(self, scenario):
        ...
        logging.debug("()_get_scenario_class_or_fail: %s", scenarios_list)
        ...
        for i, _ in enumerate(self.ego_vehicles):
            if self.ego_vehicles[i]:
                if not self._args.waitForEgo:
                    logging.debug("Destroying ego vehicle {%d : %s}", self.ego_vehicles[i].id, self.ego_vehicles[i].attributes)
    ...
    def _prepare_ego_vehicles(self, ego_vehicles):
        ...
        """
        logging.debug("()_prepare_ego_vehicles: waitForEgo=%s", self._args.waitForEgo)
        if not self._args.waitForEgo:
            for vehicle in ego_vehicles:
                logging.debug("requesting new actor(%s, %s, %s)",vehicle.model,vehicle.transform,vehicle.rolename)
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if CarlaDataProvider.get_map().name != town and CarlaDataProvider.get_map().name != "OpenDriveMap":
            logging.error("The CARLA server uses the wrong map: {}".format(CarlaDataProvider.get_map().name))
            logging.error("This scenario requires to use map: {}".format(town))
            return False
    ...
    def _load_and_run_scenario(self, config):
        ...
        """
        logging.info(' ScenarioRunner::_load_and_run_scenario(%s %s)',config.name, config.town)
        result = False
        ...
        if self._args.agent:
            ...
            try:
                ...
                traceback.print_exc()
                logging.warning("Could not setup required agent due to {}".format(e))
                self._cleanup()
                return False
        ...
        try:
            ...
            logging.debug("loading_scenario(%s)", scenario)
            self.manager.load_scenario(scenario, self.agent_instance)
            logging.debug("run_scenario(%s)", scenario)
    ...
    def _run_scenarios(self):
        ...
        """
        logging.info(' ScenarioRunner::_run_scenarios()')
        result = False
    ...
    def run(self):
        ...
        logging.info("No more scenarios .... Exiting")
        return result
...
def main():
    ...
    logging.basicConfig(format='**SR******%(levelname)s: %(message)s', level=log_level)
