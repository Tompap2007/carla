...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        logging.debug("blueprint:::::%s",blueprint)
        if random_location:
            actor = None
            while not actor:
            ...
            spawn_points_of_map = CarlaDataProvider._map.get_spawn_points()
             # ~ for p in spawn_points_of_map: logging.debug("all spawn points %s",p)
#            spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
            logging.debug("_spawn_point %s",_spawn_point)
