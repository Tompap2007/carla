...
class ManeuverOppositeDirection(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        # ~ self._first_vehicle_location = 4
        # ~ self._second_vehicle_location = self._first_vehicle_location + 2
        ...
        logging.debug("new_config.trigger_points %s", config.trigger_points[0])
    ...
    def _initialize_actors(self, config):
        ...
        """
        logging.debug("reference waypoint      [ %s ]",self._reference_waypoint)
        logging.debug("first_vehicle_location  [ %s ]",self._first_vehicle_location)
        logging.debug("second_vehicle_location [ %s ]",self._second_vehicle_location)
        ...
        logging.debug("                     ")
        logging.debug("first_actor_waypoint  [ %s ]",first_actor_waypoint)
        logging.debug("second_actor_waypoint [ %s ]",second_actor_waypoint)
        logging.debug("                     ")
        # ~ second_actor_waypoint = second_actor_waypoint.get_left_lane()
        second_actor_waypoint_left = second_actor_waypoint.get_left_lane()
        if second_actor_waypoint_left==None:
            logging.warning("second_actor_waypoint_left is nullptr +++++++++++++++++++++++++++++++++++++++")
            second_actor_waypoint_left = carla.Transform(
        ...
        if self._obstacle_type == 'vehicle':
            ...
            logging.debug("requesting new actor(%s, %s).....1",first_actor_model,second_prop_transform)
        ...
        logging.debug("requesting new actor(%s, %s).....2",first_actor_model,first_actor_transform)
        ...
        logging.debug("requesting new actor(%s, %s)......3",'vehicle.audi.tt',second_actor_waypoint.transform)
