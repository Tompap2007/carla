...
def get_waypoint_in_distance(waypoint, distance):
    ...
    traveled_distance = 0
    loops = 0
    logging.debug("\t waypoint is intersection %d, %s %s",waypoint.is_intersection, traveled_distance, distance)
    while not waypoint.is_intersection and traveled_distance < distance:
        waypoint_new = waypoint.next(1.0)[-1]
        traveled_distance += waypoint_new.transform.location.distance(waypoint.transform.location)
        waypoint = waypoint_new
        loops+=1
    logging.debug("get_waypoint_in_distance results %d, %s, %s, %d", waypoint.is_intersection, traveled_distance, distance, loops)
